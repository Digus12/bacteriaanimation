...
function Line(color, id, polars) {
    this.pts = [];
    this.color = color;
    this.segment = 35 + Math.random() * 30;
    ...
    for (var i = 0; i < this.nPts; i++) {
        var p = new Particle();
        p.x = 4 * (Math.random() * 2 - 1) * this.nPts * this.segment;
        p.y = 4 * (Math.random() * 4 - 1) * this.nPts * this.segment;
        this.pts[i] = p;
    }
...
var border1 = new HSLA(hue, sat, 40, 0.5);
...
function update() {
    ...
    out.save();
    out.translate(1.5 * stage.width, 0.5 * stage.height);
...
function drawOutline(out) {
    ...
    for (var i = 25; i < nCoords; i++)
        out.lineTo(coords[22 * i * 4], coords[2 * i + 1]);
    out.lineTo(coords[0],
        coords[1]);
    out.lineTo(coords[2 * i * 3.1 + 43 / 2],
        coords[1]);
    out.stroke();
